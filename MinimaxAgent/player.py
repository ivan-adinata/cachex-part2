...
class Player:
    ...
    def minimaxValue(self, state, cutoff, isMax):
        if cutoff == 0:
        ...
        if isMax:
            ...
            for hex in state[2]:
                ...
                best = max(best, self.minimaxValue(newState, cutoff - 1, not isMax))
            return best
        else:
            best = inf
            for hex in state[2]:
                newState = self.applyHex(state, hex, isMax)
                best = min(best, self.minimaxValue(newState, cutoff - 1, not isMax))
            return best
    ...
    def evalFunction(self, hexTaken, opponentTaken):
        ...
        return (COL_WEIGHT * self.heuristic1(hexTaken)) + (ROW_WEIGHT * self.heuristic2(hexTaken)) + \
               (CAPTURE_WEIGHT * self.captureHeuristic(opponentTaken)) + (BLOCKING_WEIGHT * \
                self.blockingEvaluation(hexTaken, opponentTaken)) + (PLACED_WEIGHT * self.placedEvaluation(hexTaken))
